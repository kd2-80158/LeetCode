  ...
  *
  ...
  It does not matter what you leave beyond the returned k (hence they are underscores).
    ...
    public int removeElement(int[] nums, int val) {
        int count = 0;  // Pointer for the new array without the value 'val'

        // Iterate through the array to check for elements equal to 'val'
        for (int i = 0; i < nums.length; i++) {
            if (nums[i] != val) {
                // If element is not equal to 'val', move it to the 'j' position
                nums[count++] = nums[i];
            }
        }

        // 'j' now represents the count of valid elements after removal
        return count;
    }
